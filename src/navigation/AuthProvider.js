  ...
  async function addNewUser(
    ...
    EThree.initialize(initializeFunction, {AsyncStorage})
      .then(async (eThree) => {
        await eThree
          .cleanup()
          .then(() => console.log('ethree cleanup success'))
        ...
        if (identityExists) {
          ...
          await eThree.resetPrivateKeyBackup().then(async () => {
            await eThree.rotatePrivateKey().then(async () => {
              let group;
              ...
              const encryptedName = await group.encrypt(
        ...
        await eThree
          .backupPrivateKey(currentUser.uid) //backupPassword
    ...
    firestore()
      ...
      .then((doc) => {
        ...
        if (
          ...
          data.latestMessage.unreadMessage != null &&
          data.latestMessage.unreadMessage != undefined
        ) {
          setNotifications((oldState) => ({
            ...oldState,
            unreadMessage: data.latestMessage.unreadMessage,
          }));
        } else {
        ...
        if (
          data.latestMessage != undefined &&
          data.latestMessage.unreadMessageFromProvider != null &&
          data.latestMessage.unreadMessageFromProvider != undefined
        ) {
          setNotifications((oldState) => ({
            ...oldState,
            unreadMessageFromProvider:
              data.latestMessage.unreadMessageFromProvider,
          }));
        } else {
          setNotifications((oldState) => ({
            ...oldState,
            unreadMessageFromProvider: false,
          }));
        }
      })
      .catch((error) => {
        ...
        if (formDataObj['dates'].length > 1) {
          for (key of keys) {
            if (key != 'dates') {
              if (
                Math.abs(
                  formDataObj[key][formDataObj[key].length - 1] -
                    formDataObj[key][formDataObj[key].length - 2],
                ) >= 2
              ) {
    ...
    <AuthContext.Provider
      value={{
        ...
        login: async (email, password) => {
          let signedIn = false;
          try {
            let {loginPassword, backupPassword} =
              EThree.derivePasswords(password);
              ...
              .then(async (userCreds) => {
                await EThree.initialize(initializeFunction, {
                  ...
                  if (!hasLocalPrivateKey)
                    await eThree
                      .restorePrivateKey(userCreds.user.uid)
                      .catch(async (e) => {
        ...
        register: async (
          ...
          if (
            ...
            alert(
              'Please make sure the following fields are not empty: name, email, phone number, password, condition, symptom timeline',
            );
            return;
          }
          try {
