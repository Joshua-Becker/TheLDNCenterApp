...
export default function FormScreen({navigation}) {
  ...
  function inSymptomVals(val) {
    if (userInfo.formVals.includes(val)) {
      return true;
    } else {
      return false;
    }
  ...
  return (
    <KeyboardAvoidingView
      ...
      <View>
        <ScrollView
          style={styles(colors).form}
          contentContainerStyle={styles(colors).formContainer}>
          <View style={styles(colors).textBox}>
            <IconButton
              style={styles(colors).notificationIcon}
              icon={'alert-circle'}
              size={30}
            />
            <Text style={styles(colors).disclaimer}>
              Please find the symptoms you are treating with LDN below and rate
              them.{' '}
            </Text>
          </View>
          <View style={styles(colors).textBox}>
          ...
          <View style={styles(colors).formQuestions}>
            <Card style={styles(colors).cards}>
              ...
              {formSymptoms.map((formItem) => {
                if (inSymptomVals(formItem.var)) {
                  return (
                    <View
                      style={styles(colors).symptomBox}
                      key={formItem.title}>
                      <Text style={styles(colors).symptomText}>
                        {formItem.title}
                      </Text>
                      <View style={styles(colors).dropdown}>
                        <DropDown
                          ...
                          setValue={(event) =>
                            handleChange(formItem.var, event)
                          }
                          list={numbers}
                          visible={open[formItem.var]}
                          dropDownContainerMaxHeight={300}
                          showDropDown={() =>
                            handleDropdown(formItem.var, true)
                          }
                          ...
                          inputProps={{
                            right: (
                              <TextInput.Icon
                                name={'menu-down'}
                                style={styles(colors).dropdownIcon}
                                size={30}
                              />
                            ),
                          }}
