...
export default function HomeScreen({navigation}) {
      ...
      .then((query) => {
        query.forEach((doc) => {
          if (
            ...
            (doc.data().type == 'general' ||
              doc.data().type == userInfo.condition) &&
            doc.data().for == 'patient'
          ) {
  ...
  async function handleLink(link) {
    Linking.openURL(link).catch(async function (error) {
      await AsyncAlert('Bad Link', '');
    });
  }
    ...
    <View style={styles(colors).container}>
      <View style={styles(colors).content}>
        <Image
          source={
            isDark
              ? require('../media/images/logo-light.png')
              : require('../media/images/logo-dark.png')
          }
        ...
        <View style={styles(colors).notifications}>
          <View style={styles(colors).notificationTitleContainer}>
            <Text style={styles(colors).notificationTitleText}>
              Notifications
            </Text>
          </View>
        ...
        <ScrollView
          style={styles(colors).newsScroll}
            ...
            <View>
              ...
              <Card style={styles(colors).card}>
                <Card.Content>
                  ...
                  <Button
                    style={styles(colors).cardView}
                    onPress={() => handleLink(latestMessage.link)}>
                    View
                  </Button>
                </Card.Content>
              </Card>
          ...
          {Boolean(Object.keys(latestMessagePharmacy).length > 0) && (
            <Card style={styles(colors).card}>
              <Card.Content>
                ...
                <Divider style={styles(colors).divider}></Divider>
                <Paragraph style={styles(colors).cardText}>
                  {latestMessagePharmacy.content}
                </Paragraph>
                <Button
                  style={styles(colors).cardView}
                  onPress={() => handleLink(latestMessagePharmacy.link)}>
                  View
                </Button>
              </Card.Content>
            </Card>
          ...
          {Boolean(Object.keys(latestMessageProvider).length > 0) && (
            <Card style={styles(colors).card}>
              <Card.Content>
                ...
                <Divider style={styles(colors).divider}></Divider>
                <Paragraph style={styles(colors).cardText}>
                  {latestMessageProvider.content}
                </Paragraph>
                <Button
                  style={styles(colors).cardView}
                  onPress={() => handleLink(latestMessageProvider.link)}>
                  View
                </Button>
              </Card.Content>
            </Card>
