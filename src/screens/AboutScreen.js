            ...
            if(data.cyj != undefined){
                ...
                for(obj of  data.cyj){
                    ...
                    if(identifiers.includes(obj['name'])){
                        ...
                        obj['value'] = await group.decrypt(obj['value'], findUserIdentity);
                    }
                    cyj.push(obj);
                }
                setWebData(cyj);
        ...
        <ScrollView style={styles(colors).scrollContainer} scrollIndicatorInsets={{ right: 1 }}>
            <View style={styles(colors).content}>
                <Card style={styles(colors).card}>
                    ...
                    {
                        webData.map(name => {
                            return <Card.Content key={name['name']}>
                                        <Title style={styles(colors).cardSubTitle}>{name['name']}</Title>
                                        <Paragraph style={styles(colors).cardText}>{name['value']}</Paragraph>
                                    </Card.Content>;
                        })
                    }
                </Card>
